...
WHERE ea.frontend_input='select' AND ea.entity_type_id=4 AND ea.attribute_code=[[attcode/attribute_code]] AND ea.is_user_defined=1
...
ON cpev.attribute_id=ea.attribute_id  AND cpev.value REGEXP CONCAT('.*,?',eaov.value,',?.*')
WHERE ea.frontend_input='multiselect' AND ea.entity_type_id=4 AND ea.attribute_code=[[attcode/attribute_code]] AND ea.is_user_defined=1
GROUP BY eao.option_id
HAVING ce=0
) as candidates);
