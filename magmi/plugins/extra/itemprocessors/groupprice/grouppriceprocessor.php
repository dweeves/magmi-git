...
class GrouppriceProcessor extends Magmi_ItemProcessor
{
	...
	public function getPluginInfo()
	{
		return array(
			...
			'author'=>'Tim Bezhashvyly,dweeves',
			'version'=>'0.0.4'
		);
	}	
	...
	public function createGroup($groupname)
	{
		...
		$sql = "INSERT INTO $cg (customer_group_code,tax_class_id) VALUES (?,?)";
	...
	public function processColumnList(&$cols, $params = null)
	{
		foreach ($cols as $col)
		{
			if (preg_match("|group_price:(.*)|", $col, $matches))
			{
				...
				$countSql = 'SELECT COUNT(*) as cnt FROM ' . $this->tablename("customer_group") . ' WHERE customer_group_code = ?';
				$groupnameCount = intval($this->selectone($sqlCount, strtoupper($groupname), "cnt"));
				...
				if ($groupnameCount == 1)
				{
					...
					$id = $this->selectone($sql, $upperGroupName, "customer_group_id");
