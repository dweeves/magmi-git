...
class CrossUpsellProducts extends Magmi_ItemProcessor
{
    ...
    public function deleteUSellItems($item, $inf)
    {
        ...
        if ($j2 != "")
        {
		  ...
		  JOIN " .
                 $this->tablename("catalog_product_entity") . " as cpe2 ON cpe2.entity_id!=cpe.entity_id AND $j2
		  WHERE cpe.sku=?";
    ...
    public function deleteCSellItems($item, $inf)
    {
        ...
        if ($j2 != "")
        {
		  ...
		  JOIN " .
                 $this->tablename("catalog_product_link_type") .
                 " as cplt ON cplt.code='cross_sell'
 		  JOIN " .
                 $this->tablename("catalog_product_link") . " as cpl ON cpl.product_id=cpe.entity_id AND cpl.link_type_id=cplt.link_type_id
 		  JOIN " .
                 $this->tablename("catalog_product_link_attribute_int") . " as cplai ON cplai.link_id=cpl.link_id
    ...
    public function setUSellItems($item, $rinfo)
    {
        ...
        
        {
			...
			FROM " . $this->tablename("catalog_product_entity") . " as cpe
			JOIN " .
                     $this->tablename("catalog_product_entity") . " as cpe2 ON cpe2.entity_id!=cpe.entity_id AND $jinf
			JOIN " .
                     $this->tablename("catalog_product_link_type") . " as cplt ON cplt.code='up_sell'
			WHERE cpe.sku=?";
    ...
    public function setCSellItems($item, $rinfo)
    {
        ...
        
        {
			...
			JOIN " .
                     $this->tablename("catalog_product_entity") . " as cpe2 ON cpe2.entity_id!=cpe.entity_id AND $j2
			JOIN " .
                     $this->tablename("catalog_product_link_type") . " as cplt ON cplt.code='cross_sell'
			WHERE cpe.sku=?";
                $sql = "INSERT IGNORE INTO " . $this->tablename("catalog_product_link") .
                     " (link_type_id,product_id,linked_product_id)  $bsql";
                $data = array_merge($joininfo["data"]["cpe2.sku"], array($item["sku"]));
